 ...
 *
 * Example useage `sendMultipleDMX([{channelA: valueA, channelB: valueB}])`
 */
function sendMultipleDMX(list = []) {
...
function switchToScene(scene) {
  ...
  const dimmerValues = Array.from(chMap, ([channel, value]) => ({
    channel,
    value,
  }));
